...
export default function HazardSchedulingSection({
  ...
  const [closureMode, setClosureMode] = useState(RunwayClosureMode.SNOW_CLEARANCE);
  ...
  }, [hazards, onHazardsChange]);

  const selectedArrivalCallsign = targetArrivalCallsign || arrivalCallsigns[0] || "";
    ...
    <Card withBorder radius="md" p="lg">
      <Stack>
        ...
        <Select
          label="Hazard type"
          value={hazardType}
          onChange={(value) => setHazardType(value || HazardType.RUNWAY_CLOSURE)}
          data={[
            {
        ...
        {hazardType === HazardType.EMERGENCY_EVENT && arrivalCallsigns.length === 0 ? (
          <Text size="sm" c="yellow.8">
            No arrival callsigns available yet. Add an arrival in the scheduling
            section first.
          </Text>
        ) : null}
