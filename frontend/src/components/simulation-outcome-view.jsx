...
const POLL_DELAYS = [1000, 2000, 4000, 8000];
    ...
    () =>
      eventLog.map((entry, index) => (
        <Table.Tr key={`${entry.timestamp || index}-${entry.type || "event"}`}>
          ...
          <Table.Td>{JSON.stringify(entry.attr || entry.details || {})}</Table.Td>
        </Table.Tr>
      )),
    [eventLog],
  );
    ...
    <Stack maw={1000} mx="auto" p="xl" gap="lg">
      ...
      {!isCompleted ? (
        <Card withBorder p="lg">
          <Group>
            <Loader size="sm" />
            <Text>Simulation is running. Polling with exponential back-off…</Text>
          </Group>
        </Card>
      ) : null}
